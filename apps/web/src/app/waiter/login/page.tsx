...
const WAITER_REQUIRED = ["tables:read", "sessions:read", "service-requests:read"];
const CASHIER_REQUIRED = ["pos:read", "pos:write", "payments:write"];

export default function WaiterLoginPage() {
  const router = useRouter();
  const [mode, setMode] = useState<"waiter" | "cashier">("waiter");
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      const required = mode === "cashier" ? CASHIER_REQUIRED : WAITER_REQUIRED;
      const hasAccess = required.every(p => res.staff.permissions.includes(p));
      if (!hasAccess) { setError(mode === "cashier" ? "You do not have cashier POS permissions." : "You do not have waiter dashboard permissions."); return; }
      setStaffToken(res.accessToken, "waiter");
      setStaffBranchId(res.staff.branchId, "waiter");
      setStaffName(res.staff.name, "waiter");
      setStaffRole(res.staff.primaryRole, "waiter");
      setStaffTenantId(res.staff.tenantId, "waiter");
      setStaffPermissions(res.staff.permissions, "waiter");
      router.push(mode === "cashier" ? "/waiter/pos" : "/waiter/dashboard");
    } catch (err: unknown) {
      ...
      <div className="mx-auto max-w-md px-5 py-10 md:py-14">
        ...
        <header className="text-center">
          ...
          <h1 className="mt-5 font-serif text-[28px] font-extrabold tracking-tight md:text-[32px]" style={{ color: "var(--ink-900)" }}>
            {mode === "cashier" ? "Cashier" : "Waiter"} <em className="font-serif italic font-medium" style={{ color: "var(--accent)" }}>Login</em>
          </h1>
          <p className="mx-auto mt-1.5 max-w-sm text-[12px] leading-relaxed" style={{ color: "var(--ink-500)" }}>
            {mode === "cashier" ? "Cashiers sign in to open the POS terminal and record payments." : "Floor staff sign in to access live tables, sessions, and orders."}
          </p>
        </header>
        ...
        <form onSubmit={handleLogin} className="mt-8 rounded-[var(--r-lg)] p-6" style={{ background: "var(--ink-0)", border: "1px solid var(--ink-200)" }}>
          <div className="space-y-3.5">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="waiter@demo.com"
          ...
          <div className="mt-4 flex gap-2">
            <button type="button" onClick={() => setMode("waiter")} className="flex flex-1 items-center justify-center gap-2 rounded-[var(--r-md)] py-2.5" style={{ background: mode === "waiter" ? "var(--accent)" : "var(--ink-0)", color: mode === "waiter" ? "var(--ink-0)" : "var(--ink-600)", border: mode === "waiter" ? "1px solid var(--accent)" : "1px solid var(--ink-200)" }}>
              <svg {...sv}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
              <span className="text-[11px] font-semibold">Waiter</span>
            </button>
            <button type="button" onClick={() => setMode("cashier")} className="flex flex-1 items-center justify-center gap-2 rounded-[var(--r-md)] py-2.5" style={{ background: mode === "cashier" ? "var(--accent)" : "var(--ink-0)", color: mode === "cashier" ? "var(--ink-0)" : "var(--ink-600)", border: mode === "cashier" ? "1px solid var(--accent)" : "1px solid var(--ink-200)" }}>
              <svg {...sv}><rect x="1" y="4" width="22" height="16" rx="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>
              <span className="text-[11px] font-semibold">Cashier</span>
            </button>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            <Link href="/waiter" className="flex items-center gap-1 text-[11px] font-medium" style={{ color: "var(--ink-600)" }}>
              <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><polyline points="15 18 9 12 15 6" /></svg>
              Back to Access Check
            </Link>
            <span className="text-[11px]" style={{ color: "var(--ink-400)" }}>Contact manager</span>
          </div>
          ...
          <div className="mt-4 pt-3" style={{ borderTop: "1px solid var(--ink-100)" }}>
            ...
            <div className="flex gap-2">
              {[
                { label: "Waiter", email: "waiter@demo.com" },
                { label: "Cashier", email: "cashier@demo.com" },
              ].map(c => (
                <button key={c.email} type="button" onClick={() => { setEmail(c.email); setPassword("password123"); setMode(c.label === "Cashier" ? "cashier" : "waiter"); }}
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          {[
            { icon: <svg {...sv}><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>, title: "Secure Staff Access", desc: "Protected by authorized floor staff only." },
            { icon: <svg {...sv}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>, title: "Role-Based Permissions", desc: "Access tables, sessions, and tools based on your role." },
            { icon: <svg {...sv}><polyline points="13 2 3 14h9l-1 8 10-12h-9l1-8" /></svg>, title: "Fast Table Service", desc: "View live tables and orders without delay." },
          ].map(c => (
