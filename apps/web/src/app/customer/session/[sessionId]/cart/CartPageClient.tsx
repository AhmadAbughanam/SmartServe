...
export default function CartPageClient({
  ...
  const allMenuItems = useMemo(() => categories?.flatMap(c => c.menuItems) ?? [], [categories]);
    ...
    return (
      <main className="flex min-h-screen flex-col" style={{ background: "var(--ink-50)" }}>
        ...
        <div className="flex flex-1 items-center justify-center px-5">
          <EmptyState icon="○" title="Your cart is empty" description="Add items from the menu to get started." action={<button onClick={() => router.push(`/customer/session/${sessionId}/menu`)} className="rounded-[12px] px-5 py-3 text-[13px] font-semibold text-white" style={{ background: COPPER }}>Browse Menu</button>} />
        </div>
      </main>
    );
  }
    ...
    <main className="flex min-h-screen flex-col" style={{ background: "var(--ink-50)" }}>
      <div className="mx-auto w-full max-w-md flex-1 flex flex-col">
        ...
        <div className="mt-4 flex-1 px-4 pb-44">
          ...
          <RecommendedForYou branchId={branchId} sessionId={sessionId} cartItems={state.items} menuItems={allMenuItems} onAddToCart={(item) => dispatch({ type: "ADD_ITEM", item })} mode="list" title="Popular with this order" />
          ...
          <div className="mt-4 rounded-[14px] p-3.5" style={{ background: "var(--ink-0)", border: "1px solid var(--ink-200)" }}>
            ...
            <div className="mt-3 flex items-center rounded-[10px] px-3 py-2.5" style={{ background: COPPER_SOFT, border: `1px solid ${COPPER_EDGE}` }}>
              <input type="text" value={specialRequest} onChange={e => setSpecialRequest(e.target.value)} onFocus={() => setRequestExpanded(true)} placeholder="Please make the pasta mild spicy. Thank you!" className="flex-1 bg-transparent text-[12px] outline-none placeholder:opacity-70" style={{ color: COPPER_INK }} maxLength={200} />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40">
        <div className="mx-auto max-w-md px-3 pb-4 pt-3" style={{ background: "linear-gradient(to top, var(--ink-50) 75%, transparent 100%)" }}>
          <button onClick={handleSubmit} disabled={submitting} className="flex w-full items-center justify-center gap-3 rounded-[14px] py-4 text-[14px] font-semibold text-white transition disabled:opacity-50 active:scale-[0.98]" style={{ background: COPPER, boxShadow: "0 12px 28px -8px rgba(194,132,29,0.55)" }}>
            {submitting ? "Sending..." : <><Cloche size={18} color="#fff" />Send to Kitchen</>}
          </button>
          <button onClick={() => router.push(`/customer/session/${sessionId}/menu`)} className="mt-2 w-full text-center text-[12px] font-semibold underline underline-offset-[3px]" style={{ color: COPPER }}>
            Continue Browsing
          </button>
        </div>
      </div>
