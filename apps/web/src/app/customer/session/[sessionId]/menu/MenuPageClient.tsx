...
function ItemSheet({ item, onClose, onAddToCart, isFavorite, onToggleFavorite }: { item: MenuItem; onClose: () => void; onAddToCart: (c: CartItem) => void; isFavorite: boolean; onToggleFavorite: (item: MenuItem) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(12,10,9,0.45)", opacity: isOpen ? 1 : 0, transition: `opacity ${SHEET_DURATION}ms ease-out` }} onClick={handleClose}>
      <div className="w-full max-w-md flex flex-col rounded-t-[24px] overflow-hidden will-change-transform" style={{ background: "var(--ink-0)", maxHeight: "92vh", transform: isOpen ? `translateY(${dragOffsetY}px)` : "translateY(100%)", transition: dragStartY === null ? `transform ${SHEET_DURATION}ms cubic-bezier(0.32, 0.72, 0, 1)` : "none", boxShadow: "0 -10px 40px -8px rgba(12,10,9,0.25)" }} onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-3.5">
            ...
            <div className="flex flex-1 flex-col min-w-0">
              <div className="flex items-start justify-between gap-2">
                ...
                <button type="button" aria-label={`${isFavorite ? "Remove" : "Add"} ${item.name} favorite`} onClick={() => onToggleFavorite(item)} className="flex h-7 w-7 flex-shrink-0 items-center justify-center" style={{ color: isFavorite ? "#dc2626" : COPPER }}>
...
export default function MenuPageClient({
  ...
  const branchId = state.branchId ?? initialBranchId;
  const branchName = state.branchName ?? initialBranchName;
  const tableCode = state.tableCode ?? initialTableCode;
  const guestCount = state.guestCount ?? initialGuestCount;
  ...
  }, [dispatch, initialBranchId, initialBranchName, initialGuestCount, initialTableCode, sessionId]);
  ...
  const { data: categories, isLoading, error } = useQuery({
    ...
    initialData: branchId === initialBranchId ? initialCategories : undefined,
  });
  ...
  return (
    <main className="relative flex min-h-screen flex-col" style={{ background: "var(--ink-50)" }}>
      <div className="mx-auto w-full max-w-md flex-1 flex flex-col">
        <div className="sticky top-0 z-30 pb-2.5" style={{ background: "var(--ink-50)" }}>
          ...
          <div className="mt-4 flex gap-2 px-4">
            <Pill icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-6 9 6v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><path d="M9 22V12h6v10" /></svg>} primary={branchName} />
            <Pill icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round"><path d="M5 7v6a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3V7" /><path d="M5 7h14" /><line x1="8" y1="20" x2="8" y2="16" /><line x1="16" y1="20" x2="16" y2="16" /></svg>} label="Table" primary={tableCode} />
            <Pill icon={<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>} primary={`${guestCount} Guest${guestCount === 1 ? "" : "s"}`} />
          </div>
          ...
          <div className="mt-3 px-4">
            <div className="flex items-center gap-2.5 rounded-[14px] px-3.5 py-3" style={{ background: "var(--ink-0)", border: "1px solid var(--ink-200)" }}>
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search dishes or drinks" className="flex-1 bg-transparent text-[13px] outline-none" style={{ color: "var(--ink-900)" }} />
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto px-4" style={{ scrollbarWidth: "none" }}>
            {[{ key: "ALL" as const, label: "All" }, ...(categories?.map(c => ({ key: c.id, label: c.name })) ?? [])].map(c => {
              const active = activeCategory === c.key;
              return (
                <button key={c.key} onClick={() => setActiveCategory(c.key)} className="flex-shrink-0 whitespace-nowrap rounded-[12px] px-4 py-2 text-[12px] font-semibold transition" style={{ background: active ? COPPER : "var(--ink-0)", color: active ? "#fff" : "var(--ink-700)", border: `1px solid ${active ? COPPER : "var(--ink-200)"}` }}>
                  {c.label}
                </button>
              );
            })}
        ...
        <div className="flex-1 px-4 pb-32 pt-2">
          {activeCategory === "ALL" && !q && (
            <RecommendedForYou branchId={branchId} sessionId={sessionId} cartItems={state.items} menuItems={allMenuItems} onAddToCart={(item) => dispatch({ type: "ADD_ITEM", item })} mode="scroll" />
          )}
            ...
            <div key={cat.id} className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {cat.menuItems.map(item => (
                  <GridCard key={item.id} item={item} onOpen={setSelectedItem} isFavorite={favoriteIds.has(item.id)} onToggleFavorite={toggleFavorite} />
                ))}
              </div>
            </div>
          ))}
      ...
      {selectedItem && (
        <ItemSheet item={selectedItem} onClose={() => setSelectedItem(null)} onAddToCart={c => dispatch({ type: "ADD_ITEM", item: c })} isFavorite={favoriteIds.has(selectedItem.id)} onToggleFavorite={toggleFavorite} />
      )}
      <MenuChatAssistant branchId={branchId} sessionId={sessionId} cartItems={state.items} menuItems={allMenuItems} onAddToCart={(item) => dispatch({ type: "ADD_ITEM", item })} open={chatOpen} onOpenChange={setChatOpen} />

      {cartCount > 0 && (
        <div className="fixed bottom-3 left-3 right-3 z-40 mx-auto max-w-md">
          <button onClick={() => router.push(`/customer/session/${sessionId}/cart`)} className="flex w-full items-center justify-between rounded-[14px] px-4 py-3.5 text-white shadow-lg active:scale-[0.98]" style={{ background: COPPER, boxShadow: "0 12px 28px -8px rgba(194,132,29,0.55)" }}>
