...
import {
  setCustomerToken, setCustomerRefresh, setCustomerPhone,
  isCustomerLoggedIn, clearCustomerAuth, getCustomerPhone,
...
interface OtpResponse { message: string; expiresInSeconds: number; _dev_otp?: string }
interface VerifyResponse { accessToken: string; refreshToken: string; user: { id: string; phone: string; name: string } }
...
export default function CustomerLoginPage() {
  ...
  async function handleVerify(e?: React.FormEvent) {
    ...
    try {
      ...
      setCustomerToken(res.accessToken); setCustomerRefresh(res.refreshToken); setCustomerPhone(res.user.phone);
  ...
  // Already-logged-in compact view
  if (loggedIn) {
    return (
  ...
  return (
    <main className="flex min-h-screen flex-col" style={{ background: "var(--ink-50)" }}>
      <div className="mx-auto flex w-full max-w-sm flex-1 flex-col px-5 pb-8 pt-5">
        {/* Back arrow */}
        <button onClick={() => router.back()} aria-label="Back" className="flex h-10 w-10 items-center justify-center rounded-[12px]" style={{ background: "var(--ink-0)", border: "1px solid var(--ink-200)" }}>
        ...
        <form onSubmit={handleSendOtp} className="mt-6">
          ...
          <div className="mt-3 flex items-start gap-2.5 rounded-[12px] p-3" style={{ background: "var(--ok-soft)", border: "1px solid #bbf7d0" }}>
            ...
            <span className="text-[11px] leading-snug pt-0.5" style={{ color: OK_DARK }}>We will send a 6-digit OTP to your registered mobile number</span>
          </div>
        </form>

        {/* OTP section — visible after Send OTP succeeds */}
        {phase === "otp" && (
